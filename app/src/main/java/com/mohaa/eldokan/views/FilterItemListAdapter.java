...
public class FilterItemListAdapter extends BaseExpandableListAdapter {
    ...
    private List<String> departmentFilter, occasionFilter,materialFilter;
//departmentFilter, occasionFilter ,materialFilter
    public FilterItemListAdapter(Context context, List<String> listDataHeader,
                                 HashMap<String, List<String>> listChildData, List<String> departmentFilter, List<String> occasionFilter , List<String> materialFilter) {
    ...
    public View getChildView(int groupPosition, final int childPosition,
        ...
        switch (groupPosition) {
            ...
            case 1:
                try {
                    if (occasionFilter.contains(childText)) {
                        img.setVisibility(View.VISIBLE);
                    } else {
                        img.setVisibility(View.GONE);
                    }
            ...
            case 2:
                try {
                    if (materialFilter.contains(childText)) {
                        img.setVisibility(View.VISIBLE);
                    } else {
                        img.setVisibility(View.GONE);
                    }
    ...
    public View getGroupView(int groupPosition, boolean isExpanded,
        ...
        switch (groupPosition) {
            ...
            case 1: // occasion
                try {
                    if (occasionFilter.size() > 0) {
                        count.setVisibility(View.VISIBLE);
                        count.setText(String.valueOf(occasionFilter.size()));
                    } else {
                        count.setVisibility(View.GONE);
                    }
                } catch (NullPointerException e) {
            ...
            case 2: // material
                try {
                    if (materialFilter.size() > 0) {
                        count.setVisibility(View.VISIBLE);
                        count.setText(String.valueOf(materialFilter.size()));
                    } else {
                        count.setVisibility(View.GONE);
                    }
                } catch (NullPointerException e) {
